  ...
  try {
    ...
    const firstVoucher = await client.query(
      `INSERT INTO vouchers (
       ...
       ) VALUES ($1, $2, $3, CURRENT_DATE, CURRENT_DATE, 10000.00,
         'Issued', $4, $4, NOW())
       RETURNING id`,
    ...
    await client.query(
      `INSERT INTO payments (
       ...
       ) VALUES ($1, $2, $3, 10000.00, CURRENT_DATE, $4)`,
      [firstVoucher.rows[0].id, transactionId, supplierId, userId]
    );

    const paidResult = await client.query(
      `UPDATE supplier_transactions
       SET balance = balance - 10000.00, payment_date = CURRENT_DATE
       WHERE id = $1
    ...
    await expectDatabaseError(client, "overpayment_test", "23514", () =>
      client.query(
        "UPDATE supplier_transactions SET balance = balance - 1.00 WHERE id = $1",
        [transactionId]
      )
    );
    ...
    assert.deepEqual(paymentHistory.rows.map((row) => row.amount), ["10000.00"]);

    await client.query(
      `UPDATE payments SET reversed_at = NOW(), reversed_by = $1,
         reversal_reason = 'Automated cancellation test'
       WHERE voucher_id = $2`,
      [userId, firstVoucher.rows[0].id]
    );
    await client.query(
      `UPDATE vouchers SET payment_status = 'Cancelled', cancelled_at = NOW()
       WHERE id = $1`,
      [firstVoucher.rows[0].id]
    );
    const reversedResult = await client.query(
      `UPDATE supplier_transactions SET balance = LEAST(amount, balance + 10000.00)
       WHERE id = $1 RETURNING balance, billing_status`,
      [transactionId]
    );
    assert.equal(reversedResult.rows[0].balance, "10000.00");
    assert.equal(reversedResult.rows[0].billing_status, "Not Paid");
    ...
    assert.equal(voucherCounts.rows[0].total, 1);
    ...
    console.log("Verified: Not Paid -> Paid using one full-balance payment.");
